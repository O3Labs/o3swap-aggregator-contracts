...
contract O3SwapETHSushiBridge is Ownable {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'O3SwapETHSushiBridge: EXPIRED');
        _;
    }
    ...
    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, UniswapV2Library.pairFor(sushiFactory, path[0], path[1]), amountIn
        );
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(address(this));
        _swapSupportingFeeOnTransferTokens(path, address(this));
        uint amountOut = IERC20(path[path.length - 1]).balanceOf(address(this)).sub(balanceBefore);
        require(amountOut >= amountOutMin, 'O3SwapETHSushiBridge: INSUFFICIENT_OUTPUT_AMOUNT');
        return amountOut;
    }
    ...
    function _swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        require(path[0] == WETH, 'O3SwapETHSushiBridge: INVALID_PATH');
        uint amountIn = msg.value.sub(fee);
        require(amountIn > 0, 'O3SwapETHSushiBridge: INSUFFICIENT_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amountIn}();
        assert(IWETH(WETH).transfer(UniswapV2Library.pairFor(sushiFactory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(address(this));
        _swapSupportingFeeOnTransferTokens(path, address(this));
        uint amountOut = IERC20(path[path.length - 1]).balanceOf(address(this)).sub(balanceBefore);
        require(amountOut >= swapAmountOutMin, 'O3SwapETHSushiBridge: INSUFFICIENT_OUTPUT_AMOUNT');
        return amountOut;
    }
    ...
    function _swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        require(path[path.length - 1] == WETH, 'O3SwapETHSushiBridge: INVALID_PATH');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, UniswapV2Library.pairFor(sushiFactory, path[0], path[1]), amountIn
        );
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IUniswapV2Pair pair = IUniswapV2Pair(UniswapV2Library.pairFor(sushiFactory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            address to = i < path.length - 2 ? UniswapV2Library.pairFor(sushiFactory, output, path[i + 2]) : _to;
